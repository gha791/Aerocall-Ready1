...
const TeamMemberRow = React.memo(function TeamMemberRow({ member, onUpdate, isCurrentUser }: { member: TeamMember, onUpdate: () => void, isCurrentUser: boolean }) {
  ...
  const handleRemove = async () => {
     if (!confirm(`Are you sure you want to remove ${member.name || member.email} from the team?`)) return;
    ...
    <TableRow>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" disabled={isCurrentUser || member.status === 'pending'}>
    ...
    <Card>
      ...
      <CardContent className="p-0">
        ...
        <Table>
          ...
          <TableBody>
            {members.map((member) => (
              <TeamMemberRow key={member.uid || member.email} member={member} onUpdate={fetchMembers} isCurrentUser={currentUserId === member.uid} />
            ))}
          </TableBody>
        </Table>
        )}
